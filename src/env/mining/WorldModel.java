...
public class WorldModel extends GridWorldModel {
    ...
    public void moveDepot(int x, int y) {
        depot = getDepot();
        remove(WorldModel.DEPOT, depot.x, depot.y);
        depot = new Location(x, y);
        data[x][y] = DEPOT;
    ...
    boolean drop(int ag) {
        ...
        if (isCarryingGold(ag)) {
            if (l.equals(getDepot())) {
                ...
                logger.warning("Agent " + (ag + 1) + " is trying the drop gold, but there is no depot at " + l.x + "x" + l.y + "!");
            }
        }
        return false;
    }
